...
- 2026-04-14 `35e60df1` 将 UE_LOG 迁移为 UE_LOGF。
- 2026-02-04 `653786a3` Electra: 移除了死代码，并在视频解码器实现返回错误时添加了上游错误报告。

### 维护评价

该插件创建于 2025 年 11 月，目前处于**活跃维护**状态。最近的提交（2026年4月）包含了功能性的现代化改进（解码器工厂重构）和代码质量提升（日志迁移、错误处理增强）。作为 `IsBetaVersion` 和 `IsExperimentalVersion` 均为 `true` 的插件，它仍在积极开发和优化中。由于其依赖特定的硬件（NVIDIA GPU）和图形API（D3D12），使用范围受限，但对于目标平台和场景，它是实现高性能视频解码的关键组件。**推荐**在需要 NVIDIA GPU 硬件解码的 Windows 项目中使用。
